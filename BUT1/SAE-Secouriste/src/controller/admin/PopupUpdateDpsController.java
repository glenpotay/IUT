 ...
 * Version: 1.0
 */
public class PopupUpdateDpsController {
    ...
    public void initialize() {
        ...
        this.horaireDeb = this.dps.getHoraireDepart();
        this.horaireFin = this.dps.getHoraireFin();

        this.sport = this.dps.getSport();
        this.site = this.dps.getSite();
    ...
    private void initializeComboBoxCompetences() {
        ...
        List<ComboBox> listComboBox = Arrays.asList(this.comboBoxCE, this.comboBoxPSE1, this.comboBoxPSE2, this.comboBoxSSA, this.comboBoxVPSP, this.comboBoxCP, this.comboBoxPBC, this.comboBoxCO,  this.comboBoxPBF);
        List<Competence> listCompetences = Arrays.asList(new Competence("CE"), new Competence("PSE1"), new Competence("PSE2"), new Competence("SSA"), new Competence("VPSP"), new Competence("CP"), new Competence("PBC"), new Competence("CO"), new Competence("PBF"));

        for (int i = 0; i < listComboBox.size(); i++) {
            int cptComp = 0;
            ArrayList<Competence> listCompetencesDps = this.besoinManagement.getBesoinByDPS(dps).getCompetences();
            for (int j = 0; j < listCompetencesDps.size(); j++) {
                if (listCompetencesDps.get(j).getIntitule().equals(listCompetences.get(i).getIntitule())) {
                    cptComp++;
                }
            }
            for (int x = 0; x <= 50 + cptComp; x++) {
                listComboBox.get(i).getItems().add(x - cptComp);
            }
        }
    ...
    public void modifDPS() {
        if (this.horaireDeb >= this.horaireFin) {
            ...
            if (this.nomTextField.getText().isEmpty()) {
                ...
                if (!dpsNameExists || this.dpsManagement.getDpsByName(this.nomTextField.getText()).getId() == this.dps.getId()) {
                    Journee journee = new Journee(this.date.getDayOfMonth(), this.date.getMonthValue(), this.date.getYear());
                    DPS newVersionDps = new DPS(this.dps.getId(), this.nomTextField.getText(), this.horaireDeb, this.horaireFin, this.site, this.sport, journee);
                    this.dpsManagement.updateDps(newVersionDps);
                } else {
    ...
    public void updateDPS() {
        this.modifDPS();
        if (dps != null) {
            ArrayList<Competence> competencesAdd = new ArrayList<>();

            for (int i = 0; i < this.comboBoxCE.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("CE"));
            }

            for (int i = 0; i < this.comboBoxCO.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("CO"));
            }

            for (int i = 0; i < this.comboBoxCP.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("CP"));
            }

            for (int i = 0; i < this.comboBoxPSE1.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("PSE1"));
            }

            for (int i = 0; i < this.comboBoxPSE2.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("PSE2"));
            }

            for (int i = 0; i < this.comboBoxPBC.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("PBC"));
            }

            for (int i = 0; i < this.comboBoxSSA.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("SSA"));
            }

            for (int i = 0; i < this.comboBoxPBF.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("PBF"));
            }

            for (int i = 0; i < this.comboBoxVPSP.getSelectionModel().getSelectedItem(); i++) {
                competencesAdd.add(new Competence("VPSP"));
            }

            ArrayList<Competence> competencesRemove = new ArrayList<>();

            for (int i = 0; i < - this.comboBoxCE.getSelectionModel().getSelectedItem(); i++) {
                System.out.println(this.comboBoxCE.getSelectionModel().getSelectedItem());
                System.out.println(-this.comboBoxCE.getSelectionModel().getSelectedItem());
                competencesRemove.add(new Competence("CE"));
            }

            for (int i = 0; i < - this.comboBoxCO.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("CO"));
            }

            for (int i = 0; i < - this.comboBoxCP.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("CP"));
            }

            for (int i = 0; i < - this.comboBoxPSE1.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("PSE1"));
            }

            for (int i = 0; i < - this.comboBoxPSE2.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("PSE2"));
            }

            for (int i = 0; i < - this.comboBoxPBC.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("PBC"));
            }

            for (int i = 0; i < - this.comboBoxSSA.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("SSA"));
            }

            for (int i = 0; i < - this.comboBoxPBF.getSelectionModel().getSelectedItem(); i++) {
                competencesRemove.add(new Competence("PBF"));
            }
